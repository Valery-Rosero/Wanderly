...
class HistoryPage extends StatefulWidget {
  ...
  const HistoryPage({super.key, required this.chatRepository, required this.usuarioId});
...
class _HistoryPageState extends State<HistoryPage> {
  ...
  Future<void> _loadHistory() async {
    ...
    try {
      final data = await widget.chatRepository.getChatHistory(userId: widget.usuarioId, limit: 500);
      setState(() {
        _history = data;
        _loading = false;
      });
  ...
  Future<void> _clearHistory() async {
    try {
      ...
      await _loadHistory();
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Historial borrado')));
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Error borrando historial: $e')));
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: _clearHistory, icon: const Icon(Icons.delete), tooltip: 'Borrar historial'),
        ],
      ),
      body: _loading
